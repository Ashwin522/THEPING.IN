	...
	public function submitPost($body, $user_to,$imageName) {
		...
		if($check_empty != "") {
			...
			$stopWords = "a about above across after again against all almost alone along already
             ...
             hate sleepy reason for some little yes bye choose looking forward";
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				if($user_logged_obj->isFriend($added_by)) {
					...
					$user_details_query = mysqli_query($this->con, "SELECT first, last , profile_pic FROM users WHERE username='$added_by'");
					$user_row = mysqli_fetch_array($user_details_query);
					$first_name = $user_row['first'];
					$last_name = $user_row['last'];
...
$comments_check_num = mysqli_num_rows($comments_check);
					...
					$str .= "<div class='status_post' onClick='javascript:toggle$id()'>
								...
								<iframe src='live.php?post_id=$id' scrolling='no'></iframe>
</div>
							</div>
	...
	if(result)
		location.reload();
						...
						<?php
							...
							echo "<p>You cannot se this post because You are not friends with this user.</p>";
							return;
						}
			...
			echo "<p>No post found if you click link it might be broken</p>";
							return;
